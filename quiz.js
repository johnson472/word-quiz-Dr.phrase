...
　　　fetch('words.json')
    ...
    　　.then(data => {
      ...
      　startButton.addEventListener("click", () => {
       ...
       　 const numQuestions = parseInt(countSelector.value, 10);　//選んだ問題数を文字列型から数値型に変換
...
　　　.catch(err => {
    ...
    function gradeQuiz() {
      ...
      document.getElementById("result").innerHTML = resultHTML + `<div class="result">スコア：${score} / ${questions.length}</div><button class="retry-button" onclick="retryQuiz()">再挑戦する</button>`;
    }

    function retryQuiz() {
      // クイズエリアを非表示、タイトル画面を表示
